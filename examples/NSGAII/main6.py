...
toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_int, n=6)  # Adjust n as needed
...
initial_population = create_initial_population(POP_SIZE, 6)
    ...
    def get_path(self, sim, app_name, message, topology_src, alloc_DES, alloc_module, traffic, from_des):
        ...
        for des in DES_dst:
            ...
            if num_paths == 0:
                continue
            if len(self.ind) == 0:
                print(f"No path found for message {message.name} from {node_src} to {dst_node}")
                continue
...
def run_yafs_simulation(individual):
    ...
    placement = LiteralPlacement(individual[:3])
    ...
    selectorPath = LiteralSelection(individual[3:], 3)
...
class NSGAIIOptimizer:
    def __init__(self, pop_size, ngen, cxpb, mutpb):
        ...
        self.toolbox.register("individual", tools.initRepeat, creator.Individual, self.toolbox.attr_int, n=6)
