...
def run_yafs_simulation(individual):
    ...
    # Filter out cases with zero messages
    if stats.df["time_latency"].count() == 0 or stats.df["time_service"].count() == 0:
...
def main():
    optimizer = NSGAIIOptimizer(pop_size=20, ngen=10, cxpb=0.7, mutpb=0.2)
