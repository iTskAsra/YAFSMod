...
# Custom placement and selection policies
...
# Create the complex application
...
# Define the default placement class
class ClusterPlacement(Placement):
    ...
    def initial_allocation(self, sim, app_name):
        ...
        value = {"model": "fog-device"}
        id_fog = sim.topology.find_IDs(value)
        value = {"model": "sensor-device"}
        id_sensors = sim.topology.find_IDs(value)
        value = {"model": "actuator-device"}
        id_actuators = sim.topology.find_IDs(value)
        ...
        for module in services.keys():
            if "Sensor" == module:
                sim.deploy_module(app_name, module, services[module], id_sensors)
            elif "Actuator" == module:
                sim.deploy_module(app_name, module, services[module], id_actuators)
            else:
                sim.deploy_module(app_name, module, services[module], id_fog)

# Define the default selection class
class First_ShortestPath(Selection):
    ...
    def get_path(self, sim, app_name, message, topology_src, alloc_DES, alloc_module, traffic, from_des):
        ...
        # Among all possible paths we choose the smallest
        bestPath = []
        bestDES = []
        print(DES_dst)
        for des in DES_dst:
...
def run_yafs_simulation(individual, is_default=False):
    ...
    if is_default:
        ...
        placement = ClusterPlacement("Cluster")
...
def main():
    optimizer = NSGAIIOptimizer(pop_size=POP_SIZE, ngen=10, cxpb=0.7, mutpb=0.2)
