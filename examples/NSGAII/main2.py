...
class ProbabilisticSelection(Selection):
    ...
    def get_path(self, sim, app_name, message, topology_src, alloc_DES, alloc_module, traffic, from_des):
        ...
        for des in DES_dst:
            ...
            num_paths = len(paths)
            probabilities = self.ind[:num_paths]
            self.ind = self.ind[num_paths:]
            probabilities = probabilities / np.sum(probabilities)  # Normalize to sum to 1
            chosen_path = np.random.choice(range(num_paths), p=probabilities)
            best_path = [paths[chosen_path]]
            best_DES = [des]
        
        return best_path, best_DES

# Define the placement class
class ProbabilisticPlacement(Placement):
    def __init__(self, ind, num_nodes):
    ...
    def initial_allocation(self, sim, app_name):
        ...
        for i, module in enumerate(services.keys()):
            ...
            probabilities = self.ind[start:end]
            probabilities = probabilities / np.sum(probabilities)  # Normalize to sum to 1
            print(f'RANGE IS {self.num_nodes} and P is {probabilities}')
            chosen_node = np.random.choice(range(self.num_nodes), p=probabilities)
...
toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_float, n=12)  # Adjust n as needed
toolbox.register("population", tools.initRepeat, list, toolbox.individual)

def normalize_probabilities(individual, num_modules, num_nodes, num_paths):
    normalized_ind = []
    ...
    for i in range(num_modules):
        ...
        total = sum(segment)
        normalized_segment = [x / total for x in segment]
        normalized_ind.extend(normalized_segment)
    
    for i in range(num_modules):
        start = num_modules * num_nodes + i * num_paths
        end = start + num_paths
        segment = individual[start:end]
        total = sum(segment)
        normalized_segment = [x / total for x in segment]
    ...
    return normalized_ind

def create_initial_population(pop_size, ind_size, num_modules, num_nodes, num_paths):
    population = toolbox.population(n=pop_size)
    for ind in population:
        ind[:] = normalize_probabilities(ind, num_modules, num_nodes, num_paths)
    return population

initial_population = create_initial_population(50, 12, 2, 3, 3)
...
def run_yafs_simulation(individual):
    ...
    placement = ProbabilisticPlacement(individual[:6], num_nodes)
    ...
    selectorPath = ProbabilisticSelection(individual[6:], 3)
...
class NSGAIIOptimizer:
    def __init__(self, pop_size, ngen, cxpb, mutpb):
        ...
        self.toolbox.register("individual", tools.initRepeat, creator.Individual, self.toolbox.attr_float, n=12)
